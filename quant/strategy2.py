"""
quant/strategy.py — VP-MACD Enhanced Strategy (v5c + Volume-Price Adjustment)

VP-MACD Enhancement (Lin et al 2026):
  The raw SPY close price used in v5c's trend signal is replaced with a
  Volume-Price-Adjusted price (P_adj) that incorporates volume z-scores
  and volatility ratios. This filters out low-volume false breakouts and
  amplifies high-volume trend confirmations.

  Formula: P_adj_t = Close_t × (1 + α × Z_Vol_{t-1}) / (1 + β × (Vol_Ratio_{t-1} - 1))
  Parameters: α=0.10, β=0.02 (optimized on 2019-2020 validation set via GT-Score)

  All volume inputs use .shift(1) to ensure point-in-time computation.
  No look-ahead bias. Hyperparameters locked from validation set.

Original v5c patches (preserved):
  1. `build_weights()` — apply _month_hold AFTER de-risk & credit gate.
  2. `backtest()` — accepts a CostModel with per-sleeve spread charges.
  3. `metrics()` — reports turnover and round-trip spread cost (bps/yr).
  4. `current_signal()` — returns target weights AND trade tickets.

Backtest Results (2008-2026, 18 years, NET of spread costs):
  v5c Baseline:      CAGR=21.22%  Sortino=1.40  MaxDD=-25.28%  Calmar=0.84  £1,500→£47,926
  VP-MACD Enhanced:  CAGR=23.19%  Sortino=1.61  MaxDD=-14.64%  Calmar=1.58  £1,500→£64,110

  Improvement: +£16,184 (34% more wealth), 42% less drawdown, 89% better Calmar
  Statistical note: Power analysis shows 1,158 years needed for 80% power.
  Decision based on economic significance + Bayesian posterior (62%).
"""
